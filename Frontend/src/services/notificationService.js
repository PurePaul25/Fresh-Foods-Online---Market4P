...
export const fetchUserNotifications = async (type = "all", page = 1, limit = 10) => {
    try {
...
export const fetchAdminNotifications = async (type = "all", page = 1, limit = 10) => {
    const params = new URLSearchParams({ type, page: String(page), limit: String(limit) })
    return await fetchAPI(`/api/notification/admin?${params}`)
}

// Send broadcast notification
export const sendBroadcastNotification = async (title, message, priority = "medium", idempotencyKey = null) => {
...
export const updateBroadcast = async (id, title, message, priority = "medium") => {
