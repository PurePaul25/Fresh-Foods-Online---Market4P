...
function Shop() {
  ...
  const categoryProducts = {
    fruits: products.filter((p) => {
      ...
      return (
        catName.includes("trái cây") ||
        catName.includes("fruit") ||
        catName.includes("táo") ||
  ...
  const bestSelling = products
    .filter((p) => p.isBestSelling || p.rating_average >= 4 || true)
    .slice(0, 10);
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="pt-15">
        ...
        >
          ...
          <div className="relative max-w-4xl">
            ...
            <div className="flex flex-wrap gap-4">
              <button className="bg-white cursor-pointer text-amber-600 px-8 py-3 rounded-full font-semibold hover:bg-amber-50 hover:scale-105 transition-all duration-300 shadow-lg">
                Mua ngay
              </button>
              <button className="border-2 cursor-pointer border-white text-white px-8 py-3 rounded-full font-semibold hover:bg-white/10 transition-all duration-300">
                Xem ưu đãi
              </button>
            </div>
          </div>
        ...
        <section
          ref={saleRef}
          className="px-6 md:px-16 lg:px-32 py-12 bg-linear-to-br from-red-50 to-orange-50"
        >
            ...
            <div className="mt-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
              {saleProducts.map((product) => (
                <div
                  ...
                  className="group bg-white rounded-2xl shadow-md p-4 transition-all duration-500 ease-out hover:shadow-2xl hover:-translate-y-3 cursor-pointer relative overflow-hidden border-2 border-red-200 flex flex-col"
                >
                  {/* Discount badge */}
                  <div className="absolute top-3 right-3 bg-linear-to-r from-red-500 to-red-600 text-white text-xs font-bold px-3 py-1.5 rounded-full z-10 shadow-lg animate-pulse">
                    -{product.discount}%
                  </div>

                  {/* Hot badge */}
                  <div className="absolute top-3 left-3 bg-linear-to-r from-orange-500 to-red-500 text-white text-xs font-bold px-2 py-1 rounded-full z-10 shadow-lg">
                    🔥 HOT
                  </div>

                  <div className="flex justify-center overflow-hidden rounded-xl bg-linear-to-b from-red-50 to-orange-50 p-4 mt-4">
                    <img
        ...
        <section
          ref={newArrivalsRef}
          className="px-6 md:px-16 lg:px-32 py-12 bg-amber-50"
        >
          <h2 className="text-3xl font-bold text-gray-800 relative inline-block">
            Sản phẩm mới
            <span className="absolute -bottom-2 left-0 w-16 h-1 bg-amber-600 rounded-full"></span>
