...
function Home() {
    ...
    const services = [
        { icon: Truck, title: "Miễn phí ship", desc: "Khi đặt hàng trên 1.000.000" },
        { icon: PhoneCall, title: "Hỗ trợ 24/7", desc: "Hỗ trợ bất cứ lúc nào" },
        { icon: RefreshCcw, title: "Hoàn trả", desc: "Hoàn trả trong vòng 3 ngày!" },
    ]
    ...
    return (
        <main>
            ...
            <div ref={heroRef} className="relative h-screen">
                <img src={trangchu} alt="Trang chủ" className="w-full h-screen object-cover" />
                <div className="absolute inset-0 bg-black/40"></div>

                {/* Banner text với animation */}
                <div
                    className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center font-bold transition-all duration-1000 ${heroVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                        }`}
                >
                    <p
                        className={`text-amber-600 text-xl transition-all duration-700 delay-200 ${heroVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"
                            }`}
                    >
                        Tươi & Sạch
                    </p>
                    <p
                        className={`text-5xl text-white my-1 transition-all duration-700 delay-400 ${heroVisible ? "opacity-100 scale-100" : "opacity-0 scale-90"
                            }`}
                    >
            ...
            <div>
                ...
                <div ref={servicesRef} className="p-20 bg-gray-100 flex items-center justify-around">
                    {services.map((service, index) => {
                        const Icon = service.icon
                        return (
                            <div
                                key={index}
                                className={`flex items-center gap-x-2 transition-all duration-700 hover:scale-105 ${servicesVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                                    }`}
                ...
                <div ref={productsRef} className="p-20">
                    <div
                        className={`text-center mb-10 transition-all duration-700 ${productsVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-8"
                            }`}
                    >
                    ...
                    <div className="flex items-center justify-evenly">
                        {products.map((product, index) => (
                            <div
                                key={index}
                                className={`shadow-lg p-10 hover:cursor-pointer transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 ${productsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
                                    }`}
                ...
                <div ref={promoRef} className="p-20 flex items-center gap-x-4 pl-52 bg-gray-100">
                    <div
                        className={`relative transition-all duration-700 ${promoVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-12"
                            }`}
                    >
                    ...
                    <div
                        className={`text-start transition-all duration-700 delay-300 ${promoVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-12"
                            }`}
                    >
                        ...
                        <div className="mb-4">
                            ...
                            <p className="leading-relaxed">
                                Dâu tây là loại trái cây đỏ mọng, có vị ngọt xen lẫn chua nhẹ, hương thơm đặc trưng và được nhiều người
                                yêu thích. Không chỉ ngon miệng, dâu tây còn giàu vitamin C, chất xơ và chất chống oxy hóa, giúp tăng
                                cường sức khỏe, làm đẹp da và hỗ trợ hệ miễn dịch.
                            </p>
                        </div>
                        <PromoCountdown />
                    </div>
